        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <h5 >Video  Lecture</h5>    <a href="javascript:void(0)" class="btn btn-success mb-2" id="create-new-category"> Add Service</a>
                </div>
                            ...
                            @foreach($services as $index=>$item)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button  data-success_url="{{url('admin/service')}}" data-token="{{ csrf_token() }}" data-url="{{ url('admin/service/delete', $item->id) }}" class="btn btn-danger delete_brand"
                                                 data-id="{{ $item->id }}"  title="Delete">Delete</button>

                                        @if($item->status == 'Inactive')
                                            <button  data-success_url="{{url('admin/service')}}" data-token="{{ csrf_token() }}" data-url="{{ url('admin/service/active', $item->id) }}" class="btn btn-success active_brand"
                                                     data-id="{{ $item->id }}"  title="Active">Active</button>
                                        @else
                                            <button  data-success_url="{{url('admin/service')}}" data-token="{{ csrf_token() }}" data-url="{{ url('admin/service/inactive', $item->id) }}" class="btn btn-warning inactive_brand"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="ajax-form-submit"   id="k"  method="POST">
                    <input type="hidden" class="success_url" value="{{url('admin/services')}}">
                    <input type="hidden" class="submit_url" value="{{url('admin/service/store')}}">
                ...
                <!-- Modal body -->
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            <label for="example-email-input" class="col-sm-3 col-form-label"> Link</label>
                            <div class="col-sm-9">
                                <input class="form-control link_modal" type="text" placeholder=" Link" name="link" required>
                                <span id="msg"></span>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#create-new-category').click(function () {
                $('#btn-save').html("Add  Service");
                $('#k').trigger("reset");
                $('#modalTitile').html("Add New Service");
            ...
            $('.edit-btn').on('click', function (e) {
                ...
                $('.k').trigger("reset");
                $('#modalTitile').html("Edit Service");
                $('#btn-save').html("Update Service");
                $('#addCategory').modal('show');
            });
